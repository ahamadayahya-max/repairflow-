...
async function sendViaBrevoApi({ to, subject, html, text, from, fromName, apiKey }) {
  ...
  const senderName  = fromName ?? (process.env.MAIL_FROM_NAME ?? 'ReparFlow')
...
async function sendViaMailpit({ to, subject, html, text, from, fromName }) {
  ...
  const fromAddress = from
    ? `"${fromName ?? 'ReparFlow'}" <${from}>`
    : `"${process.env.MAIL_FROM_NAME ?? 'ReparFlow'}" <${process.env.MAIL_FROM_ADDRESS ?? 'noreply@reparflow.local'}>`
