...
async function sendViaBrevoApi({ to, subject, html, text, from, fromName, apiKey }) {
  const senderEmail = from ?? (process.env.MAIL_FROM_ADDRESS ?? 'noreply@repairflow.fr')
  const senderName  = fromName ?? (process.env.MAIL_FROM_NAME ?? 'RepairFlow')
...
async function sendViaMailpit({ to, subject, html, text, from, fromName }) {
  ...
  const fromAddress = from
    ? `"${fromName ?? 'RepairFlow'}" <${from}>`
    : `"${process.env.MAIL_FROM_NAME ?? 'RepairFlow'}" <${process.env.MAIL_FROM_ADDRESS ?? 'noreply@repairflow.local'}>`
